...
import com.hexanovate.familytree.ui.admin.DateRangeSelectorHelperAdmin
import com.hexanovate.familytree.ui.admin.DateSelectorHelperAdmin
import com.hexanovate.familytree.ui.admin.GenderRadioButtonsAdmin
import com.hexanovate.familytree.ui.admin.marriage.EditMarriageActivityAdmin
import com.hexanovate.familytree.ui.admin.marriage.MarriageAdapterAdmin
 ...
 */
class EditPersonActivityAdmin : AppCompatActivity() {
    ...
    private lateinit var genderRadioButtons: GenderRadioButtonsAdmin

    private lateinit var datesSelectorHelper: DateRangeSelectorHelperAdmin
    ...
    private fun assignUiComponents() {
        ...
        personImageView.setOnClickListener {
            startActivityForResult(
                    PersonActivityCommonsAdmin.getImagePickerIntent(this),
                    PersonActivityCommonsAdmin.REQUEST_PICK_IMAGE
            )
        }
        ...
        genderRadioButtons = GenderRadioButtonsAdmin(
                this,
        ...
        marriageRecyclerView = findViewById<RecyclerView>(R.id.recyclerView_marriages).apply {
            ...
            layoutManager = LinearLayoutManager(this@EditPersonActivityAdmin)
        }
    ...
    private fun setupDatePickers() {
        val dateOfBirthHelper = DateSelectorHelperAdmin(this, findViewById(R.id.editText_dateOfBirth))
        val dateOfDeathHelper = DateSelectorHelperAdmin(this, findViewById(R.id.editText_dateOfDeath))
        datesSelectorHelper = DateRangeSelectorHelperAdmin(dateOfBirthHelper, dateOfDeathHelper)
    }

    private fun setupLayout() {
        PersonActivityCommonsAdmin.setupNameInputError(
    ...
    private fun setupMarriageList() {
        ...
        val marriageAdapter = MarriageAdapterAdmin(this, person.id, marriages)
    ...
    private fun setupChildrenList() {
        ...
        val personAdapter = PersonAdapterAdmin(children)
    ...
    private fun chooseChildDialog() {
        ...
        val personAdapter = PersonAdapterAdmin(getPotentialChildren())
        personAdapter.onItemClick { _, person ->
            addChildToUi(person)
            dialog.dismiss()
        }
        ...
        builder.setView(recyclerView)
                ...
                .setPositiveButton(R.string.action_create_new) { _, _ ->
                    val intent = Intent(this, EditPersonActivityAdmin::class.java)
                    startActivityForResult(intent, PersonActivityCommonsAdmin.REQUEST_CREATE_CHILD)
                    dialog.dismiss()
                }
                .setNegativeButton(android.R.string.cancel) { _, _ ->  }
    ...
    private fun chooseMarriageDialog() {
        ...
        val marriageAdapter = MarriageAdapterAdmin(this, person.id, potentialMarriages)
        marriageAdapter.onItemClick { _, marriage ->
        ...
        builder.setView(recyclerView)
                ...
                .setPositiveButton(R.string.action_create_new) { _, _ ->
                    val intent = Intent(this@EditPersonActivityAdmin, EditMarriageActivityAdmin::class.java)
                            .putExtra(EditMarriageActivityAdmin.EXTRA_WRITE_DATA, false)
                            .putExtra(EditMarriageActivityAdmin.EXTRA_EXISTING_PERSON, person)
                    startActivityForResult(intent, PersonActivityCommonsAdmin.REQUEST_CREATE_MARRIAGE)
                    dialog.dismiss()
                }
                .setNegativeButton(android.R.string.cancel) { _, _ ->  }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            PersonActivityCommonsAdmin.REQUEST_CREATE_CHILD -> if (resultCode == Activity.RESULT_OK) {
                // User has successfully created a new child from the dialog
                val child = data!!.getParcelableExtra<Person>(CreatePersonActivityAdmin.EXTRA_PERSON)
                addChildToUi(child)
            }

            PersonActivityCommonsAdmin.REQUEST_CREATE_MARRIAGE -> if (resultCode == Activity.RESULT_OK) {
                // User has successfully created a new marriage from the dialog
                val marriage =
                        data!!.getParcelableExtra<Marriage>(EditMarriageActivityAdmin.EXTRA_MARRIAGE)
                addMarriageToUi(marriage)
            }

            PersonActivityCommonsAdmin.REQUEST_PICK_IMAGE -> {
                val bitmap = PersonActivityCommonsAdmin.getImageFromResult(
