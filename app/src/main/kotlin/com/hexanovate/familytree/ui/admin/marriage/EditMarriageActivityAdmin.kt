...
import com.hexanovate.familytree.ui.admin.PersonSelectorHelperAdmin
 ...
 */
class EditMarriageActivityAdmin : AppCompatActivity() {
    ...
    private lateinit var person1Selector: PersonSelectorHelperAdmin
    private lateinit var person2Selector: PersonSelectorHelperAdmin
    ...
    private fun assignUiComponents() {
        person1Selector = PersonSelectorHelperAdmin(this, findViewById(R.id.editText_person1)).apply {
            onCreateNewPerson = { _, _ ->
                val intent = Intent(this@EditMarriageActivityAdmin, CreatePersonActivityAdmin::class.java)
                startActivityForResult(intent, REQUEST_CREATE_PERSON_1)
            }
        }
        person2Selector = PersonSelectorHelperAdmin(this, findViewById(R.id.editText_person2)).apply {
            onCreateNewPerson = { _, _ ->
                val intent = Intent(this@EditMarriageActivityAdmin, CreatePersonActivityAdmin::class.java)
    ...
    private fun initDatePickers() {
        val startDateHelper = DateSelectorHelperAdmin(this, findViewById(R.id.editText_startDate))
        val endDateHelper = DateSelectorHelperAdmin(this, findViewById(R.id.editText_endDate))
        datesSelectorHelper = DateRangeSelectorHelperAdmin(startDateHelper, endDateHelper)
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            REQUEST_CREATE_PERSON_1 -> if (resultCode == Activity.RESULT_OK) {
                val newPerson1 = data!!.getParcelableExtra<Person>(CreatePersonActivityAdmin.EXTRA_PERSON)
                person1Selector.person = newPerson1
            }
            REQUEST_CREATE_PERSON_2 -> if (resultCode == Activity.RESULT_OK) {
                val newPerson2 = data!!.getParcelableExtra<Person>(CreatePersonActivityAdmin.EXTRA_PERSON)
    ...
    private fun uploadData(){
        val stringrequest = object : StringRequest(Method.POST, "https://hexanovate.000webhostapp.com/manager/addMarriages.php",
                Response.Listener { response ->
                    try{
                        val jsonObject = JSONObject(response)
                        //Toast.makeText(applicationContext,""+jsonObject.getBoolean("error"), Toast.LENGTH_SHORT).show()
                        if (!jsonObject.getBoolean("error")) {
                            Toast.makeText(this, "Marriage added", Toast.LENGTH_SHORT).show()
                        } else {
            ...
            override fun getParams(): Map<String, String> {
                ...
                params["person1_id"] = ""+marriage!!.person1Id
                params["person2_id"] = ""+marriage!!.person2Id
                params["startDate_dayOfMonth"] = ""+marriage!!.startDate.dayOfMonth
                params["startDate_month"] = ""+marriage!!.startDate.month
                params["startDate_year"] =""+marriage!!.startDate.year
                params["endDate_dayOfMonth"] = ""+ marriage!!.endDate!!.dayOfMonth
                params["endDate_month"] = ""+ marriage!!.endDate!!.month
                params["endDate_year"] = ""+ marriage!!.endDate!!.year
                params["placeOfMarriage"] =""+marriage!!.placeOfMarriage
